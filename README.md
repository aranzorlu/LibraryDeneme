...
** Check the `ConnectionStrings` in `appsettings.json` files under the `LibraryDeneme.Blazor` and `LibraryDeneme.DbMigrator` projects and change it if you need.
**
...
* Run `LibraryDeneme.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.
...
* `LibraryDeneme.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
** `LibraryDeneme.Blazor`: ASP.NET Core Blazor Server application that is the essential web application of the solution.
